...
def test_failure_analysis_outputs_write_manifest(tmp_path) -> None:
    analysis = analyze(benchmark_fixture(), golden_fixture(), lexical_index_fixture(), benchmark_sha256="abc")
    ...
    write_outputs(
        ...
        source_artifacts=["retrieval_results.json", "golden.jsonl", "lexical_index.json"],
    )
    manifest = json.loads(manifest_path.read_text(encoding="utf-8"))

    assert manifest["artifact_type"] == "evaluation_result"
    assert manifest["evaluation_type"] == "retrieval_failure_analysis"
...
def test_benchmark_manifest_resolves_results_with_checksum(tmp_path) -> None:
    benchmark_path = tmp_path / "retrieval_results.json"
    benchmark_path.write_text(json.dumps(benchmark_fixture()), encoding="utf-8")
    manifest_path = tmp_path / "retrieval_results.manifest.json"
    manifest_path.write_text(
        json.dumps(
            {
                "artifact_type": "evaluation_result",
                "evaluation_type": "retrieval_benchmark",
                "outputs": {"results": str(benchmark_path)},
                "checksums": {"results": sha256_file(benchmark_path)},
            },
            sort_keys=True,
        ),
        encoding="utf-8",
    ...
    assert benchmark_path_from_manifest(manifest_path) == benchmark_path


def test_benchmark_manifest_rejects_checksum_mismatch(tmp_path) -> None:
    benchmark_path = tmp_path / "retrieval_results.json"
    benchmark_path.write_text(json.dumps(benchmark_fixture()), encoding="utf-8")
    manifest_path = tmp_path / "retrieval_results.manifest.json"
    manifest_path.write_text(
        json.dumps(
            {
                ...
                "evaluation_type": "retrieval_benchmark",
                "outputs": {"results": str(benchmark_path)},
                "checksums": {"results": "not-the-real-digest"},
            },
            sort_keys=True,
        ),
        encoding="utf-8",
    ...
    try:
        benchmark_path_from_manifest(manifest_path)
