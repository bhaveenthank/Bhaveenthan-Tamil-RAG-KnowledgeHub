...
from evaluation.evaluation_artifacts import EVALUATION_SCHEMA_VERSION, write_evaluation_manifest
from evaluation.runtime_adapters import (
    DEFAULT_LEXICAL_WEIGHT,
    DEFAULT_SEMANTIC_WEIGHT,
    DEFAULT_VECTOR_MANIFEST,
    HybridRetriever,
    LexicalRetriever,
    SemanticRetriever,
)
...
def write_manifest(
    ...
    *,
    ...
    evaluated_queries = sum(
        int(mode.get("metrics", {}).get("queries_evaluated") or 0)
        for mode in results.values()
        if isinstance(mode, dict)
    )
    return write_evaluation_manifest(
        evaluation_type="retrieval_benchmark",
        producer="evaluation.evaluate_retrieval",
        record_count=evaluated_queries,
        outputs=outputs,
        manifest_path=manifest_path,
        source_artifacts=source_artifacts or [],
        modes=sorted(
            mode for mode, payload in results.items() if isinstance(payload, dict) and "metrics" in payload
        ),
        warnings=[
            f"{mode}:{payload['status']}"
            for mode, payload in sorted(results.items())
...
def main(argv: list[str] | None = None) -> int:
    ...
    write_outputs(
        ...
        source_artifacts=[str(args.golden_queries)],
    )
