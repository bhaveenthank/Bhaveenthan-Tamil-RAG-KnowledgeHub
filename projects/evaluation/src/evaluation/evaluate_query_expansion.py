...
from evaluation.runtime_adapters import HybridRetriever, QueryExpander
...
def write_outputs(
    ...
    *,
    ...
    write_evaluation_manifest(
        evaluation_type="query_expansion",
        producer="evaluation.evaluate_query_expansion",
        record_count=int(summary["queries_tested"]),
        outputs={"results": output, "report": report},
        manifest_path=manifest_path,
        source_artifacts=source_artifacts or [],
        modes=[str(evaluation.get("retrieval_mode") or "hybrid")],
        metrics=summary,
        warnings=[
            f"degraded:{summary['degraded']}"
            for _ in [None]
            if int(summary.get("degraded") or 0) > 0
        ],
    )
...
def main(
    ...
    write_outputs(
        ...
        source_artifacts=[str(args.enriched)],
    )
    summary = evaluation["summary"]
    print(
